...
public class MapManager : MonoBehaviour { //todo; 이거 싱글톤으로
    ...
    public void CreateMap()
    {
        ...
        {
            Map[x] = new Hex[MapSizeY*2+1][];
            for (int y = 0; y <= MapSizeY; y++)
            {
                Map[x][y] = new Hex[MapSizeZ * 2+1];
                for (int z = 0; z <= MapSizeZ; z++)
                {
                        Map[x][y][z + MapSizeZ] = ((GameObject)Instantiate(GO_hex)).GetComponent<Hex>();
                        Vector3 pos = GetWorldPos(x, y, z);
                        Map[x][y][z + MapSizeZ].transform.position = pos;
                        Map[x][y][z + MapSizeZ].SetMapPos(x, y, z);
    ...
    public bool HilightMoveRange(Hex start,int moveRange)
    {
        ...
        {      
            for (int y = 0; y <= MapSizeY; y++)
            {
                for (int z = 0; z <= MapSizeZ; z++)
                {
                    if (Map[x+MapSizeX][y+MapSizeY][z+MapSizeZ].Passable==true)
                    {
                        ...
                        if(distance<=moveRange && distance!=0)
                        {
                            ...
                            {
                                Map[x + MapSizeX][y + MapSizeY][z + MapSizeZ].SetColor(1);                         
                               highLighedCount++;
                           }
                    ...
                    if (Map[x + MapSizeX][y + MapSizeY][z + MapSizeZ].Passable == true)
                    {
                        ...
                        if (distance <= AtkRange && distance != 0)
                        {
                            ...
                            if(isExit==true)
                            {
                                ...
                                {
                                    Map[x + MapSizeX][y + MapSizeY][z + MapSizeZ].SetColor(4);
                                    highLighedCount++;
                                }
        ...
        for (int x = 0; x <= MapSizeX; x++)
        {
            for (int y = 0; y <= MapSizeY; y++)
            {
                for (int z = 0; z <= MapSizeZ; z++)
                {
                        ...
                        Map[x + MapSizeX][y + MapSizeY][z + MapSizeZ].SetColor(Map[x + MapSizeX][y + MapSizeY][z + MapSizeZ].onto); 
                }
            }
        }
